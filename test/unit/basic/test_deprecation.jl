...
@testitem "v1 keyword arguments map to v2 plugin and mutation configuration" begin
    using SymbolicRegression

    # `annealing`/`alpha` auto-inject a SimulatedAnnealingPlugin
    options = Options(; annealing=true, alpha=0.5)
    sa = only(p for p in options.plugins if p isa SimulatedAnnealingPlugin)
    @test sa.alpha == 0.5

    options = Options(; annealing=false)
    @test !any(p -> p isa SimulatedAnnealingPlugin, options.plugins)

    # `use_frequency`/`use_frequency_in_tournament` configure the AdaptiveParsimonyPlugin
    options = Options(; use_frequency=false)
    ap = only(p for p in options.plugins if p isa AdaptiveParsimonyPlugin)
    @test ap.tournament == true
    @test ap.mutation_acceptance == false

    options = Options(; use_frequency=false, use_frequency_in_tournament=false)
    @test !any(p -> p isa AdaptiveParsimonyPlugin, options.plugins)

    # `perturbation_factor`/`probability_negate_constant` rebuild the default ConstantMutation
    options = Options(; perturbation_factor=0.5, probability_negate_constant=0.2)
    cm = only(m for (m, w) in options.mutations if m isa ConstantMutation)
    @test cm.perturbation_factor == 0.5
    @test cm.probability_negate == 0.2

    # camelCase spellings still map onto the same configuration
    options = @test_deprecated Options(; useFrequency=false)
    ap = only(p for p in options.plugins if p isa AdaptiveParsimonyPlugin)
    @test ap.mutation_acceptance == false

    options = @test_deprecated Options(; annealing=false, probNegate=0.2)
    @test !any(p -> p isa SimulatedAnnealingPlugin, options.plugins)
    cm = only(m for (m, w) in options.mutations if m isa ConstantMutation)
    @test cm.probability_negate == 0.2
end
